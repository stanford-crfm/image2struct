# Source: https://github.com/stanford-crfm/helm/blob/main/src/helm/benchmark/scenarios/vision_language/image2struct/utils_latex.py # noqa: E501
    ...
    if num_try_remaining > 0:
        ...
        if undefined_search:
            ...
            if num_try_remaining < MAX_NUM_TRIES or fixed_code == original_latex_code:
                ...
                assert (
                    TEX_INCLUDES in fixed_code
                ), "TEX_INCLUDES should be present in the code"
...
def latex_to_image(
    ...
    if (
        TEX_BEGIN_DOCUMENT not in original_latex_code
        and TEX_BEGIN_FILE not in original_latex_code
    ):
    ...
    if documentclass_search:
        ...
        original_latex_code = original_latex_code.replace(
            f"\\documentclass{{{documentclass}}}", TEX_BEGIN_FILE
        )
    else:
    ...
    try:
        ...
        if (
            str(e)
            == "No available builder could be instantiated. Please make sure LaTeX is installed."
        ):
            raise Exception(
            ...
            return handle_latex_error(
                e, original_latex_code, assets_path, crop, resize_to, num_try_remaining
            )
